...
public class UnivMatchController {
    ...
    @PostMapping("/match")
    public ResponseEntity<Response<?>> matchUniv(@RequestBody MailDto dto) throws Exception {
        try {
            ...
            if (code == null || code.isBlank()) {
                ...
                return ResponseEntity.ok(new Response<>("true", "인증 코드가 이메일로 전송되었습니다." +
                        "\n메일이 오지 않았다면 메일 주소나 스팸메일함을 확인해보세요.", null));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new Response<>("false", "학교 인증 중 오류가 발생했습니다.", null));
        }
    }
}
